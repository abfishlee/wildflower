...
### 현재 앱 기준 식별/학습 데이터 흐름

현재 Flutter 파일럿 앱은 실제 모델 연동 전 단계로, 아래 흐름을 먼저 고정해두었습니다.

1. 사용자가 `야생화 찍기` 또는 `사진 올리기`로 이미지를 선택합니다.
2. 앱은 식별 시도 정보를 `flower_identification_attempts`에 남길 수 있는 구조로 후보 분석을 시작합니다.
3. 모델/API가 반환한 후보는 `flower_identification_candidates`에 순위, 신뢰도, 매칭 근거와 함께 저장합니다.
4. 사용자가 후보 중 하나를 직접 확정하면 `flower_discoveries`에 수집 카드가 생성됩니다.
5. 사용자 확정 라벨과 사진은 `training_photo_candidates`에 학습 후보로 쌓입니다.
6. 운영자는 `vw_training_dataset_queue`에서 검수 대상을 확인하고, 승인된 사진만 향후 모델 학습 데이터셋으로 사용합니다.

이 구조의 핵심은 "AI가 맞혔다"로 끝내지 않고, 사용자가 최종 확정한 라벨을 따로 남겨서 시간이 지날수록 자체 식별 정확도를 올릴 수 있게 하는 것입니다.

---

### 실데이터가 필요한 시점

아래 작업부터는 제공처 데이터가 필요합니다.

| 필요한 시점 | 필요한 데이터 | 이유 |
| :--- | :--- | :--- |
| 도감 확장 | 국명, 학명, 설명, 개화 시기, 희귀도, 기본 이미지 | 지도/카드/검색의 기준 DB가 됩니다. |
| 실제 식별 API 연동 | 학명 기준 매칭 가능한 야생화 목록 | 외부 모델 결과를 우리 DB의 꽃 카드로 연결해야 합니다. |
| 자체 모델 학습 | 꽃별 다각도 사진, 라벨, 촬영 지역/계절, 검수 상태 | 사진 수가 많고 라벨이 정확해야 분류 정확도가 올라갑니다. |
| 품질 검수 | 오인식 사례, 비슷한 꽃 묶음, 전문가 검수 결과 | 후보 재랭킹과 사용자 안내 문구를 개선합니다. |

실데이터가 들어오기 전까지는 DB 스키마, API 계약, UI 흐름, 검수 대기열을 먼저 완성할 수 있습니다.

---

### 운영자가 바로 확인할 DB 조회 뷰

`db/init.sql`에는 다음 조회 뷰를 추가했습니다.

| View | 용도 |
| :--- | :--- |
| `vw_training_dataset_queue` | 학습 후보 사진과 사용자 확정 라벨 검수 대기열 |
| `vw_flower_collection_stats` | 꽃별 수집 수, 공개 카드 수, 학습 후보 수, 평균 신뢰도 |
| `vw_region_collection_progress` | 지역명 기준 수집 현황과 학습 후보 현황 |
| `vw_public_flowerbed_cards` | 꽃밭 공개 카드, 반응 수, 댓글 수 통합 조회 |

이 뷰들은 관리자 화면, 대시보드, 데이터 검수 도구를 만들 때 바로 API 응답 원천으로 사용할 수 있습니다.
